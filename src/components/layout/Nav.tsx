...
export function Nav() {
  // Les entrées de `site.nav` sont des ancres de l'accueil (`#projets`). Depuis
  // une autre page, une ancre seule ne mène nulle part : elle cherche une
  // section qui n'existe pas et le clic ne fait rien. On les préfixe alors par
  // `/`. Défaut apparu avec la page FAQ, la première page distincte du site.
  const surAccueil = usePathname() === "/";
  const lien = (href: string) => (surAccueil ? href : `/${href}`);
  ...
  return (
    ...
    >
      <div className="container-site flex h-full items-center justify-between gap-6">
        ...
        <SpotlightNavbar
          items={site.nav.map((i) => ({ ...i, href: lien(i.href) }))}
          activeIndex={section}
